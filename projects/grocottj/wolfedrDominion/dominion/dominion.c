/* Drew Wolfe's Dominion.c file from Assignment #2 branch.
*
*/
...
//A2 Refactor CLEAN CARD | NO BUGS
    ...
    // provides another action turn
      state -> numActions++;

    // played card to discard pile
...
//A2 Refactor
    ...
    //BUG: Card provides another buy turn 
    //state->numBuys++;      
...
//A2 Refactor
    ...
    { // when the deck is empty, reshuffle the discard and make it the deck
      if (state -> deckCount[currentPlayer] < 1) 
      {
        shuffle(currentPlayer, state);
      }
      drawCard(currentPlayer, state);
        // defines the drawn card as the top card
      ...
      else 
      {
        temphand[z] = cardDrawn;
            // removes the most recently drawn (top) card
...
//A2 Refactor
  ...
  int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState * state, int handPos, int * bonus) {
    ...
    //was commented out
	//int drawntreasure=0; // commented out as part of A2 Refactor
    //int cardDrawn; // commented out as part of A2 Refactor
    //int z = 0;// this is the counter for the temp hand // commented out as part of A2 Refactor
    //was commented out
    ...
    switch (card) 
    {
      ...
      return adventurer_card(state);
        ...
        // A2 Refactor    
      case council_room:
      return council_room_card(handPos, state);

        // A2 Refactor  
      case smithy:
      return smithy_card(handPos, state);

        // A2 Refactor  
      case village:
      return village_card(handPos, state);
